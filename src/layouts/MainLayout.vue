<template>
  ...
  <q-layout view="hHr lpR fFr">
    <q-header>
      <q-toolbar class="bg-blue-grey-9 text-white">
        <q-toolbar-title class="flex row">
          <RouterLink
            class="main-layout__home-link"
            :to="{ name: 'Home' }"
            replace
          >
            <img src="~/assets/logo.svg" height="30" />
            <span class="q-ml-xs">GooCollage</span>
          </RouterLink>
        </q-toolbar-title>
        <q-item-label v-if="savingIndicator" caption>Saving...</q-item-label>
        <q-circular-progress
    ...
    <SideDrawer v-model="leftDrawerOpen"></SideDrawer>
...
export default defineComponent({
  ...
  computed: {
    ...mapState(useAlbumStore, {
      savingIndicator: 'getSavingIndicator',
    }),
    availableFullscreen() {
